import { Status, BundleType } from "../loader/pie-loader";
import { PieItemElement } from "../interface";
import { getElementsToLoad } from "../loader/legacy-loader";

describe("getElementsToLoad", () => {
  const registry = {};
  ...
  it("only adds packages that are not in the registry", () => {
    const result = getElementsToLoad(
      els,
      BundleType.player,
      withBundle("player", registry)
    );
    expect(result).toBeDefined();
    expect(Object.keys(result).length).toEqual(1);
    expect(result["math-inline"]).toBeDefined();

    const result2 = getElementsToLoad(
      els,
      BundleType.player,
      withBundle("editor", registry)
    );
    expect(result).toBeDefined();
  ...
  it("add packages if registry entry is missing needed constructor", () => {
    ...
    const result2 = getElementsToLoad(
      els,
      BundleType.editor,
      withBundle("clientPlayer", registry)
    );
