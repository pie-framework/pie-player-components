...
import { newE2EPage, setContent } from "../../test-util/e2e-page-workaround";
...
describe("pie-author", () => {
  ...
  it("renders", async () => {
    await setContent(page, '<pie-author config="evan"></pie-author>');
  ...
  it("creates new models if models is empty ", async () => {
    await setContent(page, "<pie-author></pie-author>");
    pieAuthor = await page.find("pie-author");
    await setupInterceptPieCloud(page, pie);
